...
void codec_init() {
	...
	RCC_I2SCLKConfig(RCC_I2S2CLKSource_PLLI2S); //commented
	...
	I2S_Cmd(CODEC_I2S, ENABLE); //commented
...
void send_codec_ctrl(uint8_t controlBytes[], uint8_t numBytes) {
	...
	while (bytesSent < numBytes) {
		...
		bytesSent++;
		while (!I2C_CheckEvent(
				CODEC_I2C, I2C_EVENT_MASTER_BYTE_TRANSMITTING)) {
...
uint8_t read_codec_register(uint8_t mapbyte) {
	...
	I2C_Send7bitAddress(
			CODEC_I2C, CODEC_I2C_ADDRESS, I2C_Direction_Transmitter);
	while (!I2C_CheckEvent(
			CODEC_I2C, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED)) {
		//wait for end of address transmission
	}
	...
	while (!I2C_CheckEvent(
			CODEC_I2C, I2C_EVENT_MASTER_RECEIVER_MODE_SELECTED)) {
		//wait for end of address transmission
	}
